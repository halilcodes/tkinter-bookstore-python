...
def get_selected_row(event):
    try:
        ...
        text_year.insert(END, selected_tuple[2])
        text_author.insert(END, selected_tuple[3])
...
def search_entry():
    ...
    for row in backend.search_db(title=title, author=author, year=year, isbn=isbn):
        listbox_records.insert(row[0]-1, row)

def add_entry():
    print("add entry")
    title, author, year, isbn = get_all_entries()
    backend.insert_to_db(title=title, author=author, year=year, isbn=isbn)
    view_all()
...
def update_selected():
    ...
    title, author, year, isbn = get_all_entries()
    backend.update_db(id=selected_id, title=title, author=author, year=year, isbn=isbn)
    view_all()

def delete_selected():
    selected_id = selected_tuple[0]
    print(f"delete selected {listbox_records.get(ACTIVE)}")
    backend.delete_db(selected_id)
    view_all()
...
listbox_records = Listbox(window, width=30)
